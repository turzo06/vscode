...
void solve()
{
    int n,x;cin >> n;map<int, int> m;
    for(int i=0;i<n;i++)cin>>x,m[x]++;
    vll v1,v2,ans;bool ok=false;
    for(auto&u:m){
        if(u.second>=2)ok=true,v1.pb(u.first),v2.pb(u.first);
        else v1.pb(u.first);
    }
    if(!ok){
        dp(n);dsc(v1);print(v1);return;
    }
    rev(v2);for(auto&u:v2)v1.pb(u);
    for(int i=0;i<v1.size()-1;i++){
        if(v1[i]!=v1[i+1])ans.pb(v1[i]);
    }
    ans.pb(v1.back());dp(ans.size());print(ans);
}
