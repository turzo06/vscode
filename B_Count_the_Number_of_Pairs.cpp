...
void solve()
{
 ...
 vector<bool> parent(200,true);
for(auto &u:m){
      if (parent[tolower(u.first)] == false or parent[toupper(u.first)]==false)continue;
     else if (isupper(u.first))
      {
          ll x = u.second;
          char d = u.first;
          ll y = m[tolower(d)];
          ...
          c1 = max(x, y) - min(x, y);
          p += c1 / 2;
          parent[tolower(u.first)]=parent[toupper(u.first)]=false;
    }
    else if (islower(u.first))
    {
          ll x = u.second;
          char d = u.first;
          ll y = m[toupper(d)];
